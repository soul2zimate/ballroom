...
public class FontResourceGenerator extends AbstractResourceGenerator {
    ...
    private void writeGetText(TreeLogger logger, ResourceContext context, JMethod method, SourceWriter sw)
        ...
        try {
            String agent = context.getGeneratorContext().getPropertyOracle().getSelectionProperty(logger, "user.agent")
                    .getCurrentValue();
            if (agent.equals("ie6") || agent.equals("ie8")) { writeSrcIE(logger, context, method, sw, urls); } else {
